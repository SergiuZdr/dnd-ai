...
> Date: 2026-07-24
> Author: Sergiu
> Related: `docs/requirements/SRD-cloud-open-model-hud.md` (this is SRD Phase P1)
> Status: **Delivered.** Retained as the spec that produced `src/ai/openaiDm.ts`
> and friends — read it as the requirement, not as a description of today's code.
> Acceptance criteria in §4.2 are ticked below. Two things landed differently
> than written: the zero-config dev default stayed pointed at local Ollama but is
> a placeholder nobody uses in practice, and PR-5 (the summarizer) turned out to
> need `'dual'` handled alongside `'openai'` — it originally checked only the
> latter, so `dual` silently fell back to Claude. Fixed 2026-07-26, with
> regression coverage in `test/ai/summarizerBackend.test.ts`.
> A follow-on **P1.5** not in this PRD added the referee+narrator split
> (`src/ai/dualDm.ts`); see `docs/design/PLAN.md`.
...
- [x] With `dmBackend:"claude"` (default) everything behaves exactly as today; `npm run typecheck` + `npm test` stay green.
- [x] `toolSchema` emits exactly 11 OpenAI function schemas whose names + required fields match `createDmTools().tools` (`test/ai/toolSchema.test.ts`).
- [x] Arg-repair turns `{dc:{type:"integer",value:15}, expr:"d20+3", reason:"x", roller:"player"}` into a valid `roll_dice` call; turns a numeric-string `amount:"25"` into `25` for `award_xp` (`test/ai/argRepair.test.ts`).
- [x] Invalid args that can't be repaired produce an `ERROR: …` tool result (fed back to the model), never a throw (`test/ai/argRepair.test.ts`).
- [x] The SSE parser assembles interleaved `content` deltas and multi-index `tool_calls` from a canned OpenAI-style stream into the right `onDelta` calls + one executed tool call (`test/ai/openaiStream.test.ts`, `test/ai/openaiDm.test.ts` — no network).
- [x] A documented live smoke (`npm run smoke:openai`) runs the opening + a force-door + a loot turn against a configured endpoint and reports a tool-calling score ≥ 3/4 (run by the owner against their chosen endpoint; not part of CI). `npm run smoke:dual` is the P1.5 equivalent.
