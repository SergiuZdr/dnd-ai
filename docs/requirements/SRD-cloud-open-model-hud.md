...
> Status: **Delivered — retained as the historical record of why this work was done.**

---

## Delivery status (added 2026-07-26)

**Read §4 onwards as the problem statement as it stood on 2026-07-23, not as a description of the code today.** Everything below is left unedited on purpose — it is the reasoning that produced the current architecture, and rewriting it would destroy that. What changed since:

| Phase | Status | Where it landed |
|---|---|---|
| **P1 — Off Claude + bug fix** | Shipped | `dmBackend:"openai"` (`src/ai/openaiDm.ts`) + arg-repair (`src/ai/argRepair.ts`) + zod→OpenAI schema derivation (`src/ai/toolSchema.ts`). Summarizer follows the same setting (`src/ai/summarizer.ts`). |
| **P1.5 — referee + narrator split** | Shipped (not in the original phasing) | `dmBackend:"dual"` (`src/ai/dualDm.ts`): a reliable tool-caller resolves mechanics, a separate permissive model writes prose. This is what removed the fade-to-black ceiling of §4.2 without putting the dice at risk. |
| **P2 — Cloud, no tunnel** | Shipped | `Dockerfile` + `fly.toml` + `docs/DEPLOY.md` (Fly and Oracle Always-Free paths). The tunnel was *kept* rather than retired — `npm run serve:remote` still works for laptop-hosted sessions. |
| **P3 — BG3 HUD** | Shipped | `src/web/index.html` — HUD layout, 3D d20 dice tray, loot toasts, quest rewards. |
| **Per-player access codes** | Shipped (not in this SRD at all) | `src/game/players.ts` + `scripts/admin/players.ts`. §1 assumed "no account system beyond the existing PIN gate"; each player now has their own revocable code and their own isolated save directory. |

Specific claims below that are **no longer true**:

- **§4 problem 5, §6 metric, §9 PR-5 — the resume/player-info bug is fixed.** `GameController.start()` now pushes state to the UI before any turn runs (`src/game/controller.ts`, the `onStateChange` call at the top of `start()`), so the character panel is populated the instant a save opens, in every mode.
- **§5.3 "drop the tunnel"** — the cloud deploy exists, but the tunnel was not removed; both paths are supported.
- **§1 "No account system beyond the existing PIN gate"** — superseded by access codes, as above.

Open item #1 ("pick and prove the free endpoint+model") was settled empirically: `meta-llama/llama-3.3-70b-instruct` as referee, `nousresearch/hermes-3-llama-3.1-70b` as narrator, on OpenRouter, gated by `npm run smoke:dual`. Open item #2 resolved to the Oracle Always-Free VM.
