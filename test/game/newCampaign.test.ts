...
describe('applyRaceBonuses', () => {
  ...
  // The web wizard renders its race cards from /api/presets and posts back the
  // preset ID, not the display name. Matching on name alone silently zeroed
  // every web-created character's racial bonuses.
  it('accepts a race ID as well as a display name, case-insensitively', () => {
    const elf = { str: 10, dex: 12, con: 10, int: 10, wis: 11, cha: 10 };
    expect(applyRaceBonuses(baseStats, 'elf')).toEqual(elf);
    expect(applyRaceBonuses(baseStats, 'Elf')).toEqual(elf);
    expect(applyRaceBonuses(baseStats, 'ELF')).toEqual(elf);
    expect(applyRaceBonuses(baseStats, ' elf ')).toEqual(elf);
    expect(applyRaceBonuses(baseStats, 'half-orc')).toEqual({ ...baseStats, str: 12, con: 11 });
  });
});

describe('findRace / canonicalRaceName', () => {
  it('resolves either spelling to the same preset', () => {
    expect(findRace('elf')?.name).toBe('Elf');
    expect(findRace('Elf')?.name).toBe('Elf');
    expect(findRace('half-orc')?.name).toBe('Half-Orc');
    expect(findRace('Half-Orc')?.id).toBe('half-orc');
  });

  it('returns undefined for something that is not a race', () => {
    expect(findRace('Dragonborn')).toBeUndefined();
    expect(findRace('')).toBeUndefined();
  });

  it('canonicalizes an id to its display name and passes unknown input through', () => {
    expect(canonicalRaceName('elf')).toBe('Elf');
    expect(canonicalRaceName('tiefling')).toBe('Tiefling');
    expect(canonicalRaceName('Elf')).toBe('Elf');
    // Never destroy a string it does not recognize -- an old save or a custom
    // race must survive a round trip unchanged.
    expect(canonicalRaceName('Dragonborn')).toBe('Dragonborn');
  });
});
...
describe('createNewCampaign', () => {
  ...
  // End-to-end guard for the defect this pair of assertions exists to catch:
  // the web wizard posts a race ID with an explicit stat assignment, and the
  // character came out with NO racial bonus and race stored as "elf" -- which
  // then rendered as "elf Rogue" in the HUD and went to the DM that way.
  it('applies the racial bonus and stores the display name when given a race ID (the web wizard path)', () => {
    const state = createNewCampaign(
      {
        name: 'Ashfall Reach',
        heroName: 'Vela Thorn',
        ...
        stats: { str: 14, dex: 15, con: 13, int: 12, wis: 10, cha: 8 },
    ...
    // Elf is +2 DEX / +1 WIS on top of the player's own assignment.
    expect(state.character.stats).toEqual({ str: 14, dex: 17, con: 13, int: 12, wis: 11, cha: 8 });
    // Rogue acBase 14 + the DEX mod capped at +2. DEX 15 and 17 both hit that
    // cap, so AC is 16 either way here -- the bonus is real but invisible in AC.
    expect(state.character.ac).toBe(16);
  });

  it('lets a racial CON bonus from an ID reach maxHp', () => {
    const state = createNewCampaign(
      {
        name: 'Deep Roads',
        heroName: 'Borin',
    ...
    expect(state.character.race).toBe('Dwarf');
    expect(state.character.stats.con).toBe(15);
    expect(state.character.stats.str).toBe(15);
    // Fighter hitBase 10 + CON 15's +2 -- one more than the +1 base CON 13 gave.
    expect(state.character.maxHp).toBe(12);
  });
